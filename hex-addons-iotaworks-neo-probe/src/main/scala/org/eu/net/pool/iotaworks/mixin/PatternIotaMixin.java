...
public class PatternIotaMixin {
    @WrapMethod(method = "display(Lat/petrak/hexcasting/api/casting/math/HexPattern;)Lnet/minecraft/network/chat/Component;")
    private static Component wrappedDisplay(HexPattern pat, Operation<Component> original) {
        ...
        return ((MutableComponent) original.call(pat)).append(buf.reverse().toString());
    }

    @WrapMethod(method = "execute")
    CastResult wrappedExecute(CastingVM vm, ServerLevel world, SpellContinuation continuation, Operation<CastResult> original) {
