...
import net.minecraft.world.effect.{MobEffectInstance as StatusEffectInstance, MobEffects as StatusEffects}
...
given [T]: Conversion[RegistryKey[Registry[T]], ? <: Registry[T]] = Registries.REGISTRY.asInstanceOf[Registry[Registry[T]]].get(_)

trait Registrar[T](val id: Identifier):
  val key: RegistryKey[Registry[T]] = RegistryKey.ofRegistry[T](id)
  lazy val registry: SimpleRegistry[T] = SimpleRegistry(key, Lifecycle.stable())
                    ...
                    ${
                      Block(a.zipWithIndex.map { p =>
                        ...
                        def coer(r: Term): Term = r.tpe.asType match
                          ...
                          case '[Seq[Iota]] => '{ OperationResult(img.withStack(s => s.dropRight(${Expr(a.size)}) ++ ${r.asExprOf[Seq[Iota]]}), util.ArrayList[OperatorSideEffect](), cont, HexEvalSounds.NORMAL_EXECUTE.get()) }.asTerm
...
class MishapTodo extends Mishap:
  ...
  override def execute(castingEnvironment: CastingEnvironment, context: Mishap.Context, list: TreeList[Iota]): TreeList[Iota] =
    castingEnvironment.getCastingEntity match
      case p: LivingEntity => p.addEffect(StatusEffectInstance(StatusEffects.CONFUSION, 30 * 20, 0))
      case null => ()
    list
...
given [T]: Conversion[Registry[T], RegistryKey[? <: Registry[T]]] = _.key()

given [T <: Iota]: Conversion[T, NbtCompound] = serialize(_)
given ServerWorld => Conversion[NbtCompound, Iota | Null] = deserialize(_, summon)

given [T: Codec, R: DynamicOps]: Conversion[T, R] = summon[Codec[T]].encodeStart(summon, _).getOrThrow()
given [T: Codec, R: DynamicOps]: Conversion[R, T] = summon[Codec[T]].decode(summon, _).getOrThrow().getFirst
