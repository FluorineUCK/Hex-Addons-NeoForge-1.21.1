...
import net.minecraft.util.crash.{CrashException, CrashReport}
...
object Patterns:
  private def catchErrors[R](f: => R): R =
    try f
    catch
      case m: Mishap => throw m
      case e: Throwable =>
        val report = CrashReport("Evaluating Hexic pattern", e)
        val cfgSection = report.addElement("Environment Configuration")
        sys.props.foreach(cfgSection.add(_, _))
        println(report.asString())
        // TODO: upload it somewhere
        e match
          case _: (NotImplementedError | MatchError) => throw MishapTodo()
          case NonFatal(e: Exception) => throw MishapInternalException(e)
          case _ => throw e
  def mkAction(body: (CastingEnvironment, ServerWorld) ?=> (CastingImage, SpellContinuation) => (OperationResult | CastResult | (CastingImage, SpellContinuation, EvalSound, Seq[OperatorSideEffect]))): Action =
    (env: CastingEnvironment, image: CastingImage, cont: SpellContinuation) =>
      catchErrors(body(using env, env.getWorld)(image, cont)) match
  ...
  def mkConstAction(argc: Int, mediaCost: Long = 0)(body: (CastingEnvironment, ServerWorld) ?=> Seq[Iota] => Seq[Iota]): Action =
    new ConstMediaAction:
      ...
      override def execute(list: util.List[? <: Iota], castingEnvironment: CastingEnvironment): util.List[Iota] =
        catchErrors(body(using castingEnvironment, castingEnvironment.getWorld)(list.toSeq))
